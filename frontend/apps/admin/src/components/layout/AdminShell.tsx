...
import { useAdminSelector, useAdminDispatch, adminLogout, setAdminAuth } from '../../store/store';
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className={`px-3 py-4 border-t border-gray-100 space-y-1`}>
        <Link href="/settings" className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-600 hover:bg-gray-100 transition-colors ${collapsed ? 'justify-center' : ''}`}>
          <Settings size={18} />
          {!collapsed && 'Settings'}
        </Link>
        <button
        ...
        <header className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-semibold text-gray-900 text-base">
              {NAV_ITEMS.find((n) => pathname === n.href || (n.href !== '/' && pathname.startsWith(n.href)))?.label || 'Dashboard'}
            </h1>
          </div>
