...
export default function CheckoutPage() {
  ...
  if (!token) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center" style={{ background: 'var(--charcoal)', gap: '1.5rem', padding: '2rem' }}>
    ...
    return (
      <div
        style={{
          ...
          justifyContent: 'center', background: 'var(--charcoal)', gap: '1.5rem', padding: '2rem', textAlign: 'center',
        }}
      >
        <div
          style={{
        ...
        >
          <Check size={32} style={{ color: '#86efac' }} strokeWidth={1.5} />
        </div>
  ...
  return (
    <div style={{ background: 'var(--charcoal)', minHeight: '100vh' }}>
      {/* Header */}
      <div style={{ padding: '4rem 1.5rem 3rem', background: 'var(--charcoal-2)', borderBottom: '1px solid rgba(250,247,240,0.06)' }}>
                ...
                <div key={s.id} style={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  >
                    <div
                      style={{
                        width: 36, height: 36,
                        border: `1px solid ${step === s.id ? 'var(--gold)' : step > s.id ? 'rgba(34,197,94,0.4)' : 'rgba(250,247,240,0.12)'}`,
                        background: step === s.id ? 'var(--gold)' : step > s.id ? 'rgba(34,197,94,0.12)' : 'transparent',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        transition: 'all 0.3s ease',
                      }}
                    >
                      {step > s.id
                        ? <Check size={14} style={{ color: '#86efac' }} strokeWidth={2} />
                        : <s.icon size={14} style={{ color: step === s.id ? 'var(--obsidian)' : 'rgba(250,247,240,0.3)' }} strokeWidth={1.5} />
                      }
                    </div>
                    <span style={{
                      fontSize: '0.55rem', letterSpacing: '0.2em', textTransform: 'uppercase',
                      color: step === s.id ? 'var(--gold)' : step > s.id ? 'rgba(34,197,94,0.7)' : 'rgba(250,247,240,0.25)',
                  ...
                  {i < STEPS.length - 1 && (
                    <div style={{ width: 40, height: 1, background: step > s.id ? 'rgba(34,197,94,0.3)' : 'rgba(250,247,240,0.08)', margin: '0 0.5rem', marginBottom: '1.5rem' }} />
                  )}
                </div>
              ))}
            </div>
        ...
        <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '2.5rem' }}>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    >
                      <option value="" style={{ background: '#1C1812' }}>Select State</option>
                      {INDIAN_STATES.map((s) => <option key={s} value={s} style={{ background: '#1C1812' }}>{s}</option>)}
                    </select>
          ...
          {step === 2 && (
            <div style={{ animation: 'fadeUp 0.4s ease forwards' }}>
                ...
                {items.map((item) => {
                  ...
                  return (
                    <div key={`${item.productId}-${item.color}`}
                      ...
                      <div style={{ position: 'relative', width: 72, height: 80, overflow: 'hidden', background: 'var(--charcoal-2)', flexShrink: 0 }}>
              ...
              {/* Address summary */}
              <div style={{ background: 'var(--charcoal-2)', border: '1px solid rgba(250,247,240,0.07)', padding: '1.5rem', marginBottom: '2.5rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <p style={{ fontSize: '0.58rem', letterSpacing: '0.25em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '0.75rem' }}>Delivering to</p>
                  ...
                  <button onClick={() => setStep(1)} style={{ fontSize: '0.58rem', letterSpacing: '0.18em', textTransform: 'uppercase', color: 'var(--gold)', background: 'none', border: 'none', cursor: 'pointer' }}>
                    Edit
                  </button>
                </div>
              </div>
            ...
            <div style={{ animation: 'fadeUp 0.4s ease forwards' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '2.5rem' }}>
                {[
                  ...
                  >
                    <div style={{
                      width: 20, height: 20, borderRadius: '50%', border: `1px solid ${paymentMethod === opt.value ? 'var(--gold)' : 'rgba(250,247,240,0.2)'}`,
                      background: paymentMethod === opt.value ? 'var(--gold)' : 'transparent', flexShrink: 0, marginTop: 2,
                      display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease',
                    }}>
                      {paymentMethod === opt.value && <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--obsidian)' }} />}
                    </div>
                    <div>
          ...
          <div style={{ background: 'var(--charcoal-2)', border: '1px solid rgba(250,247,240,0.07)', padding: '1.75rem' }}>
            ...
            <div style={{ marginTop: '1.25rem', paddingTop: '1.25rem', borderTop: '1px solid rgba(250,247,240,0.08)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
                ...
                <span style={{ fontSize: '0.78rem', color: subtotal >= 150000 ? 'var(--gold)' : 'rgba(250,247,240,0.6)' }}>
...
function SectionTitle({ num, label }: { num: string; label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2.5rem' }}>
      <span style={{ fontFamily: "'Playfair Display',serif", fontSize: '0.8rem', color: 'var(--gold)', fontWeight: 300 }}>{num}</span>
      <div style={{ flex: 1, height: 1, background: 'rgba(250,247,240,0.07)' }} />
